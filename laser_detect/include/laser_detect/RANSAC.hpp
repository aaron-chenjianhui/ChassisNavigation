                             ...
                             std::vector<bool>& vote_index)
{
  ...
  if ((numDataObjects < numForEstimate) ||
      ...
      (desiredProbabilityForNoOutliers <= 0.0)) return 0;
  ...
  numVotesForBest = 0;         // initalize with 0 so that the first computation
                               // which gives any type of fit will be set to
                               // best
  numTries        = allTries;  // initialize with the number of all possible
                               // subsets
  ...
  for (i = 0; i < numTries; i++) {
    ...
    for (l = 0; l < numForEstimate; l++) {
      ...
      int selectedIndex =
        (int)(((float)rand() / (float)RAND_MAX) * maxIndex + 0.5);
    ...
    // get the indexes of the chosen objects so we can check that this sub-set
    // hasn't been
    // chosen already
    ...
    std::pair<typename std::set<int *, SubSetIndexComparator>::iterator,
    ...
    if (res.second == true) { // first time we chose this sub set
      ...
      // selected data is a singular configuration (e.g. three colinear points
      // for
      // a circle fit)
      ...
      // continue checking data until there is no chance of getting a larger
      // consensus set
      // or all the data has been checked
      for (j = 0;
           j < numDataObjects &&
           numVotesForBest - numVotesForCur < numDataObjects - j + 1; j++) {
      ...
      if (numVotesForCur > numVotesForBest) {
        ...
        if (numVotesForBest == numDataObjects) i = numTries;
        else { // update the estimate of outliers and the number of iterations
               // we need
          denominator =
            log(1.0 -
                pow((double)numVotesForCur / (double)numDataObjects,
                    (double)(numForEstimate)));
          numTries = (int)(numerator / denominator + 0.5);

          // there are cases when the probablistic number of tries is greater
          // than all possible sub-sets
  ...
  // release the memory
  typename std::set<int *,
                    SubSetIndexComparator>::iterator it = chosenSubSets.begin();
  typename std::set<int *,
                    SubSetIndexComparator>::iterator chosenSubSetsEnd =
    chosenSubSets.end();
  ...
  for (j = 0; j < numDataObjects; ++j) {
    vote_index.push_back(bestVotes[j]);
  }
...
double RANSAC<T, S>::compute(std::vector<S>& parameters,
                             ParameterEstimator<T, S> *paramEstimator,
                             std::vector<T>& data, std::vector<bool>& vote_index)
{
  unsigned int numForEstimate = paramEstimator->numForEstimate();
  std::vector<T *> leastSquaresEstimateData;
  int    numDataObjects  = data.size();
  ...
  short *curVotes        = new short[numDataObjects]; // one if data[i] agrees
                                                      // with the current model,
                                                      // otherwise zero
  short *bestVotes       = new short[numDataObjects]; // one if data[i] agrees
                                                      // with the best model,
                                                      // otherwise zero
  ...
  if (numDataObjects < numForEstimate) return 0;
...
template<class T, class S>
void RANSAC<T, S>::computeAllChoices(ParameterEstimator<T, S> *paramEstimator,
                                     std::vector<T>& data,
                                     bool *bestVotes,
                                     bool *curVotes,
                                     int& numVotesForBest,
...
template<class T, class S>
void RANSAC<T, S>::estimate(ParameterEstimator<T, S> *paramEstimator,
                            std::vector<T>& data,
                            bool *bestVotes,
                            bool *curVotes,
                            int& numVotesForBest,
                            int *arr)
{
  ...
  for (j = 0; j < numForEstimate;
       j++) exactEstimateData.push_back(&(data[arr[j]]));
  paramEstimator->estimate(exactEstimateData, exactEstimateParameters);

  // singular data configuration
  if (exactEstimateParameters.size() == 0) return;
...
unsigned int RANSAC<T, S>::choose(unsigned int n, unsigned int m)
{
  ...
  for (i = numeratorStart, numerator = 1; i <= n; i++) numerator *= i;

  for (i = 1, denominator = 1; i <= denominatorEnd; i++) denominator *= i;
  ...
  if ((denominator > std::numeric_limits<double>::max()) ||
      ...
      (static_cast<double>(std::numeric_limits<unsigned int>::max()) <
       result)) return std::numeric_limits<unsigned int>::max();
  else return static_cast<unsigned int>(result);
}
