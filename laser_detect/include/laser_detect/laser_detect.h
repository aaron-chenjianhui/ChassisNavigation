...
public:
    ...
    void DataSelect(double min_angle, double max_angle, lidar_data_type& lidar_data_in,
                    lidar_data_type& lidar_data_out);
    void DataFilter(lidar_data_type& lidar_data_in, lidar_data_type& lidar_data_out);
    void DataIgn(vec_data_type& ign_angle, lidar_data_type& lidar_data_in,
                 lidar_data_type& lidar_data_out);
    // path_param: (nx, ny, ax, ay);        dest_pose: (dest_x, dest_y, dest_theta)
    bool WallDetect(lidar_data_type& lidar_data, vec_data_type& l_param, vec_data_type& r_param,
                                        vec_data_type& f_param);
    void DataProcess(vec_data_type& l_param, vec_data_type& r_param,
                                        vec_data_type& f_param, vec_data_type& ori_pose);
    //    void
    ...
    void ResultDisp(lidar_data_type& lidar_data, vec_data_type& ori_pose, vec_data_type& left_line_param,
                                     vec_data_type& right_line_param, vec_data_type& front_line_param);
    void BWFilter(lidar_filter_type& filter_data_in, lidar_filter_type& filter_data_out,
    ...
    void ScanToPoint(const S& s, P& p){
        ...
        for (typename S::const_iterator it = s.begin(); it != s.end(); ++it){
            ...
            p.push_back(Point2D((it->second)*cos(it->first)*1000, (it->second)*sin(it->first)*1000));
        }
    }
