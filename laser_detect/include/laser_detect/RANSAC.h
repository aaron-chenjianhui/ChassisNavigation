 ...
 * number of
 *    data objects required for an exact estimate and N is the total number of
 * data objects.
 ...
 *
 * The class template parameters are T - objects used for the parameter
 * estimation
...
public:
   ...
   * desiredProbabilityForNoOutliers is not in (0,1)
   * @param paramEstimator An object which can estimate the desired parameters
   * using either an exact fit or a
   *                       least squares fit.
   ...
   * the selected subsets doesn't contain an
   ...
   * @return Returns the percentage of data used in the least squares estimate.
   *
   * NOTE: This method should be used only when n choose k is small (i.e. k or
   *(n-k) are approximatly equal to n)
   *
   */
  static double compute(std::vector<S>& parameters,
                        ParameterEstimator<T, S> *paramEstimator,
...
private:
  ...
  static void         computeAllChoices(ParameterEstimator<T, S> *paramEstimator,
                                        std::vector<T>& data,
                                        bool *bestVotes,
                                        bool *curVotes,
                                        int& numVotesForBest,
  ...
  static void estimate(ParameterEstimator<T, S> *paramEstimator,
                       std::vector<T>& data,
                       bool *bestVotes,
                       bool *curVotes,
                       int& numVotesForBest,
