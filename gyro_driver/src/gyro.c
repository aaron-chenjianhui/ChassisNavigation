...
int DataTranslate(uchar  buf[],
                  size_t buf_size,
                  float *ang_vel_out,
                  float *accel_ahead_out,
                  float *ang_out) {
  if (9 == buf_size) {
    ...
    // Positive
    if (0x00 == vel_symbol) {
      vel_int = (buf[0] & 0x0f) * 10000 + (buf[1] >> 4) * 1000 + (buf[1] & 0x0f) *
                100 +
    ...
    if (0x00 == accel_symbol) {
      accel_int = (buf[3] & 0x0f) * 10000 + (buf[4] >> 4) * 1000 +
                  (buf[4] & 0x0f) * 100 +
    ...
    //
    if (0x00 == ang_symbol) {
      ang_int = (buf[6] & 0x0f) * 10000 + (buf[7] >> 4) * 1000 + (buf[7] & 0x0f) *
                100 +
...
//       int i;
//       for (i = 0; i < n-4; ++i){
//         if (0x68 == recv_buf[i] && 0x05 == recv_buf [i+1] && 0x00 ==
// recv_buf[i+2]
//         && 0x28 == recv_buf[i+3])
