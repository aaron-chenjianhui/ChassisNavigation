...
AMCLIntfc::AMCLIntfc() {
  m_init_puber = m_nh.advertise<geometry_msgs::PoseWithCovarianceStamped>(
    "initialpose",
    1000);
  m_crs_puber     = m_nh.advertise<geometry_msgs::Pose2D>("coarse_pose", 1000);
  m_update_client =
    m_nh.serviceClient<std_srvs::Empty>("request_nomotion_update");
  m_laser_suber = m_nh.subscribe("scan", 1000,
                                 &AMCLIntfc::LaserCallback, this);
  m_amcl_suber =
    m_nh.subscribe("amcl_pose", 1000, &AMCLIntfc::AMCLCallback, this);
