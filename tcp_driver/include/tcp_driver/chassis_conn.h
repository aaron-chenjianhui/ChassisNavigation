...
int  WRSelect(bool& readOK, bool& writeOK);
...
int  SendMsg(const pc_status_t & pc_status, const LocationData & location_data, const ContainerData& container_data);
int  SendMsg(const pc_status_t & pc_status, const LocationData& location_data);
int  SendMsg(const pc_status_t& pc_status);

// Overload function for receiving message
int  RecvMsg(chassis_status_t& chassis_status, OdomData        & odom_data);

void parseTCPData(const u_char *buffer_data, chassis_status_t& chassis_status, OdomData        & odom_data);
